...
def get_model_and_batches_for_eval_time(dataset_key, seq_len, horizon, estimation_samples):
    ...
    args = argparse.Namespace(
        device=str(device), dataset=dataset_key, years='2019', model_name='dcrnn', seed=2023,
        bs=1, seq_len=seq_len, horizon=horizon, input_dim=3, output_dim=1, mode='test',
        max_epochs=100, patience=30, n_filters=64, max_diffusion_step=2, filter_type='doubletransition',
        num_rnn_layers=2, cl_decay_steps=2000, lrate=1e-2, wdecay=0, clip_grad_value=5,
    )
    ...
    model = DCRNN(node_num=node_num, input_dim=args.input_dim, output_dim=args.output_dim,
                  device=device, adj_mx=adj_mx, n_filters=args.n_filters,
                  max_diffusion_step=args.max_diffusion_step, filter_type=args.filter_type,
                  num_rnn_layers=args.num_rnn_layers, cl_decay_steps=args.cl_decay_steps)
    model.to(device)
    batches = []
...
def main():
    if '--eval-time' in sys.argv:
        ...
        eval_time_common.run_eval_time_for_experiment(
            model_name='dcrnn', model_path='experiments/dcrnn',
