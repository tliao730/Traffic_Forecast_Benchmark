"""
DLinear (Decomposition-Linear) for traffic forecasting using gift_eval data format.
...
Run from benchmark/:
  uv run --project fm/moirai python -m linear.dlinear --year 2018
"""
...
class MovingAvg(nn.Module):
    ...
    def forward(self, x):
        ...
        x = torch.cat([front, x, end], dim=1)           # (B, L+pad*2, 1)
        x = self.avg(x.permute(0, 2, 1)).permute(0, 2, 1)  # (B, L, 1)
        return x


class DLinearModel(nn.Module):
    """Decomposition-Linear: separate linear projections for trend and residual."""

    def __init__(self, context_length: int, prediction_length: int, kernel_size: int = 25):
        super().__init__()
    ...
    def forward(self, x):
        # x: (B, L, 1)
        trend    = self.moving_avg(x).squeeze(-1)         # (B, L)
        seasonal = (x - self.moving_avg(x)).squeeze(-1)   # (B, L)
        return self.linear_trend(trend) + self.linear_seasonal(seasonal)  # (B, pred_len)
...
def main():
    ...
    predictor = DLinearPredictor(checkpoints=checkpoints, args=args, device=device)
