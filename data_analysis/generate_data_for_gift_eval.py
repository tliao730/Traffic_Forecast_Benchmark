...
def build_time_features(index, add_time_of_day, add_day_of_week):
    ...
    if add_time_of_day:
        ...
        frac_day = (
            (index - index.normalize()) / pd.Timedelta(days=1)
        ).astype(float)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--freq', type=str, default='15T', help='sampling frequency, aligned with pandas offsets (e.g., 15T)')
    parser.add_argument('--output_dir', type=str, default='./gift_eval_datasets', help='root directory to save the arrow dataset')
    parser.add_argument('--overwrite', action='store_true', help='overwrite existing arrow dataset directory if it exists')
