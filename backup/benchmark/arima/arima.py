...
class ARIMAPredictor:
    ...
    def predict(self, dataset, num_samples=100):
        ...
        for item in dataset:
            ...
            if self.count % 100 == 0:
                print(
                    f"  Processed {self.count} (success: {self.success_count}, "
            ...
            try:
                ...
                with time_limit(self.timeout):
                    ...
                    if len(history.shape) > 1:
                        ...
                        for dim in range(num_dims):
                            ...
                            if np.any(np.isnan(history_1d)):
                                history_1d = (
                                    pd.Series(history_1d).ffill().bfill().values
                                )
                                if np.any(np.isnan(history_1d)):
                            ...
                            if len(history_1d) < 10:
                                # Use last value
                                forecast_1d = np.full(
                                    self.prediction_length, history_1d[-1]
                                )
                            else:
                                # Use auto_arima
                                model = auto_arima(
                    ...
                    if len(forecast_mean.shape) > 1:
                        samples = np.tile(
                            forecast_mean[np.newaxis, :, :], (num_samples, 1, 1)
                        )
                    else:
                        samples = np.tile(
                            forecast_mean[np.newaxis, :], (num_samples, 1)
                        )
                ...
                try:
                    ...
                    if len(history.shape) > 1:
                        fallback = np.tile(
                            history[-1][np.newaxis, :], (self.prediction_length, 1)
                        )
                        samples = np.tile(
                            fallback[np.newaxis, :, :], (num_samples, 1, 1)
                        )
                    else:
